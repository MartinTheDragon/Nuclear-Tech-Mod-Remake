...
class RBMKConsoleScreen(
    ...
    override fun mouseClicked(x: Double, y: Double, button: Int): Boolean {
        ...
        if (mouseX in guiLeft + 30 .. guiLeft + 57 && mouseY in guiTop + 138 .. guiTop + 165) {
            if (az5Lid) {
                az5Lid = false
                Minecraft.getInstance().soundManager.play(SimpleSoundInstance.forUI(NTechSoundsCore.rbmkAz5Cover.get(), 0.5F))
            } else if (lastEmergyPress + 3000L < System.currentTimeMillis()) {
                Minecraft.getInstance().soundManager.play(SimpleSoundInstance.forUI(NTechSoundsCore.rbmkShutdown.get(), 1F))
