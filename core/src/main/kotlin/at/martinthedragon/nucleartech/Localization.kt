...
import at.martinthedragon.nucleartech.coremodules.minecraft.network.chat.TranslatableComponent
...
object LangKeys {
    ...
    @Suppress("NOTHING_TO_INLINE")
    private inline fun of(category: String, path: String, namespace: String = MOD_ID) = TranslationKey(category, path, namespace)
}

@JvmInline
value class TranslationKey(val key: String) : Supplier<Component> {
    constructor(category: String, path: String, namespace: String = MOD_ID) : this("$category.$namespace.$path")
...
fun ntmTranslationString(key: String) = "$MOD_ID.$key"
