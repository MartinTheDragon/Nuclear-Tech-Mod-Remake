...
class BlastingJeiRecipeCategory(guiHelper: IGuiHelper) : IRecipeCategory<BlastingRecipe> {
    ...
    override fun getTitle() = "Blasting"

    override fun getTitleAsTextComponent() = TranslationTextComponent("jei.${NuclearTech.MODID}.category.blasting")
    ...
    override fun draw(recipe: BlastingRecipe, matrixStack: MatrixStack, mouseX: Double, mouseY: Double) {
        blastArrow.draw(matrixStack, 23, 19)
        drawExperience(recipe, matrixStack)
    }

    private fun drawExperience(recipe: BlastingRecipe, matrixStack: MatrixStack) {
        val experience = recipe.experience
        if (experience > 0) {
            val experienceString = TranslationTextComponent("jei.${NuclearTech.MODID}.category.blasting.experience", experience)
