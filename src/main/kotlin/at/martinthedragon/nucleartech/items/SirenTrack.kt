...
class SirenTrack(val sound: SoundEvent, val volume: Float, val loop: Boolean, track: String, val color: Int) :
    ...
    val trackType: ITextComponent = TranslationTextComponent("item.nucleartech.$track.type")
        .withStyle(Style.EMPTY.withColor(Color.fromRgb(color)))
    val trackRange: ITextComponent = TranslationTextComponent("item.nucleartech.$track.range")
        .withStyle(Style.EMPTY.withColor(Color.fromRgb(color)))
