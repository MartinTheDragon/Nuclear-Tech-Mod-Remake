...
open class EffectItem(val effectTypes: List<EffectTypes>, properties: Properties, val radPerSecond: Float = 0f) : Item(properties) {
    enum class EffectTypes(val effectInfo: ITextComponent) {
        Radioactive(TranslationTextComponent("item.ntm.any.tooltip.radiation.radioactive").withStyle(TextFormatting.GREEN)),
        Blinding(TranslationTextComponent("item.ntm.any.tooltip.radiation.blinding").withStyle(TextFormatting.DARK_AQUA)),
        Hot(TranslationTextComponent("item.ntm.any.tooltip.radiation.hot").withStyle(TextFormatting.GOLD))
    }
